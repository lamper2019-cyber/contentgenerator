    ...
    <>
      <div className="glass-card p-6 animate-fade-in">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <span className="text-base font-bold tracking-tight text-foreground block">Slides</span>
        ...
        <button
          ...
          disabled={isLoading}
          className="btn-ghost w-full py-3.5 text-sm press-scale"
        >
          {isLoading ? (
        ...
        {error && (
          <div className="mt-4 p-4 rounded-2xl border border-danger/30 bg-danger/10 text-sm text-danger animate-fade-in">
            {error}
          </div>
        )}

        {output && (
          <div className="mt-5 animate-fade-in" style={{ background: 'rgba(255,255,255,0.03)', borderRadius: '18px', border: '1px solid rgba(255,255,255,0.06)', overflow: 'hidden' }}>
            <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
              ...
              <div className="flex items-center gap-1.5">
                <button
                  onClick={handleCopy}
                  className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-xl text-muted hover:text-foreground hover:bg-[rgba(255,255,255,0.06)] transition-all duration-200 press-scale"
                >
                  {copied ? (
                    <>
                ...
                <button
                  onClick={handleGenerate}
                  disabled={isLoading}
                  className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-xl text-muted hover:text-foreground hover:bg-[rgba(255,255,255,0.06)] transition-all duration-200 disabled:opacity-50 press-scale"
                >
        ...
        {isLoading && (
          <div className="mt-5 animate-fade-in" style={{ background: 'rgba(255,255,255,0.03)', borderRadius: '18px', border: '1px solid rgba(255,255,255,0.06)', overflow: 'hidden' }}>
            ...
            <p className="text-sm text-muted text-center pb-4">Building your slide deck...</p>
          </div>
        )}
      </div>
